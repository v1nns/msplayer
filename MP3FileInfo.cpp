...
MP3FileInfo::MP3FileInfo(MP3FileInfo& file)
{
	/* Fileinfo */
	szFilename = file.szFilename;
	bHasLyrics = file.bHasLyrics; 
	bHasV1Tag = file.bHasV1Tag;
	bHasV2Tag = file.bHasV2Tag;
	nFileSize = file.nFileSize;

	/* Headerinfo */
	nCbrBitRate = file.nCbrBitRate; 
	nVbrBitRate = file.nVbrBitRate; 
	nBitRate 	= file.nBitRate; 	
	nSampleRate = file.nSampleRate; 
	nLength		= file.nLength;		
	
	szBitRate = file.szBitRate;
	szMpegLayer = file.szMpegLayer;
	szMpegVersion = file.szMpegVersion;
	szChannelMode = file.szChannelMode;

	/* V1 up */
	szArtist = file.szArtist;
	szTitle = file.szTitle;
	szAlbum = file.szAlbum;
	szComment = file.szComment;
	szTrack = file.szTrack; 
	nTrack = file.nTrack;
	szYear = file.szYear; 
	nYear = file.nYear;
	szGenre = file.szGenre;

	/* V2 only */
	szComposer = file.szComposer;
	szCopyright = file.szCopyright;
	szEncodedBy = file.szEncodedBy;
	szOriginalArtist = file.szOriginalArtist;
	szURL = file.szURL;
}
